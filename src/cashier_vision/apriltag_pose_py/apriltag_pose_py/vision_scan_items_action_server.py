...
class VisionScanItemsActionServer(Node):
    def __init__(self):
        ...
        default_specs_path = os.path.expanduser(
            "~/cobot_ws/src/apriltag_pose_py/config/item_specs.json"
        )
        self.declare_parameter("item_specs_json_path", default_specs_path)
        ...
        self.item_specs_json_path = self.get_parameter("item_specs_json_path").value
        ...
        if self.use_compressed:
            self.create_subscription(
                CompressedImage, self.color_topic, self.on_color_compressed, 10
            )
    ...
    def load_item_specs(self, json_path: str) -> Dict[str, dict]:
        try:
            ...
            self.get_logger().info(
                f"Loaded item specs: {len(normalized)} entries from {json_path}"
            )
            return normalized
        ...
        try:
            ...
            while time.time() < end_time:
                ...
                for conf, item in self.latest_items:
                    track_key = self.make_track_key(item.item_id, item.name, item.x, item.y)
                ...
                if boxes is not None:
                    for box in boxes:
                        try:
                            ...
                            item_id = qr_text.strip() if qr_text else f"{cls_name}_{cx}_{cy}"
                            track_key = self.make_track_key(item_id, cls_name, x_mm, y_mm)
                            ...
                            if workspace_ready:
                                ...
                                if spec:
                                    cv2.putText(
                                        debug_img,
                                        f"spec w={item.width:.1f} d={item.depth:.1f} h={item.height:.1f} dur={item.durability}",
                                        (x1, min(debug_img.shape[0] - 10, y2 + 40)),
                                        cv2.FONT_HERSHEY_SIMPLEX,
                                        0.45,
                                        (255, 255, 0),
                                        1,
                            ...
                            if qr_text:
                                cv2.putText(
                                    debug_img,
                                    f"QR:{qr_text}",
                                    (x1, min(debug_img.shape[0] - 60, y2 + 60)),
                                    cv2.FONT_HERSHEY_SIMPLEX,
                                    0.55,
                                    (255, 0, 255),
                                    2,
    ...
    def make_track_key(self, item_id: str, name: str, x: float, y: float) -> str:
        if item_id and not item_id.startswith(f"{name}_"):
            return f"qr:{item_id}"

        gx = int(round(x / 30.0))
        gy = int(round(y / 30.0))
        return f"{name}_{gx}_{gy}"
