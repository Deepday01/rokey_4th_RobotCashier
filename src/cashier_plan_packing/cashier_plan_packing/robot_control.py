...
#     self.get_logger().info(
#         f"[PICK] complete: ({task.pick_x:.3f}, {task.pick_y:.3f}, {task.pick_z:.3f})"
#     )


# def place_object(self, task):
#     from DSR_ROBOT2 import get_current_posx, mwait
...
#     self.get_logger().info(
#         f"[PLACE] complete: ({task.place_x:.3f}, {task.place_y:.3f}, {task.place_z:.3f})"
#     )
...
#     def __init__(self):
#         super().__init__("pick_and_place_action_server", namespace=ROBOT_ID)
...
#         self.get_logger().info("ActionServer ready: /pick_and_place")
...
#         if len(goal_request.tasks) == 0:
#             self.get_logger().warn("Reject goal: no tasks")
#             return GoalResponse.REJECT

#         self.get_logger().info(f"Goal accepted: task_count={len(goal_request.tasks)}")
#         return GoalResponse.ACCEPT
...
#     def publish_feedback(self, goal_handle, current_index: int, current_item: str, progress: float):
#         fb = PickAndPlace.Feedback()
#         fb.current_index = current_index
#         fb.current_item = current_item
...
#             move_ready()
#             self.publish_feedback(goal_handle, 0, "ready", 0.0)

#             for i, task in enumerate(tasks):
#                 if goal_handle.is_cancel_requested:
#                     goal_handle.canceled()
#                     result.success = False
#                     result.message = f"Canceled before task {i}"
#                     self.get_logger().warn(result.message)
#                     return result

#                 item_name = task.item_name if task.item_name else f"item_{i}"
#                 self.get_logger().info(f"[TASK {i+1}/{total}] start: item={item_name}")

#                 pick_object(self, task)
#                 progress = ((i + 0.5) / total) * 100.0
#                 self.publish_feedback(goal_handle, i, item_name, progress)

#                 if goal_handle.is_cancel_requested:
#                     goal_handle.canceled()
#                     result.success = False
#                     result.message = f"Canceled after pick of task {i}"
#                     self.get_logger().warn(result.message)
#                     return result

#                 place_object(self, task)
#                 progress = ((i + 1) / total) * 100.0
#                 self.publish_feedback(goal_handle, i + 1, item_name, progress)

#                 self.get_logger().info(f"[TASK {i+1}/{total}] done: item={item_name}")
...
#             goal_handle.succeed()
#             result.success = True
#             result.message = f"Pick and place completed: {total} task(s)"
#             return result
