...
N_SELECT = 6          # 여기만 바꾸면 됨
GREEDY = True         # True면 argmax, False면 sampling
RANDOM_SEED = 42
...
def main():
    ...
    print(f"Placed: {len(placements)}/{len(objects)}")

    visualize_solution(
        placements,
        BASKET_SIZE,
        title=f"Packing result | placed {len(placements)}/{len(objects)} | reward {total_reward:.3f}",
    )
