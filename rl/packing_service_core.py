...
# User settings
# =========================
CHECKPOINT_PATH = "checkpoint_heightmap_attention_policy_candidate_split_v2.pth"
...
def objects_from_request_items(items):
    ...
    for episode_index, item in enumerate(items):
        sx = int(round(float(item.x)))
        sy = int(round(float(item.y)))
        sz = int(round(float(item.z)))
        durability = int(item.durability)

        if sx <= 0 or sy <= 0 or sz <= 0:
            raise ValueError(f"invalid size at index {episode_index}: ({sx}, {sy}, {sz})")

        objects.append({
            "name": f"item_{episode_index}",
            "size": (sx, sy, sz),
